...
// CRITICAL: Trust proxy setting for Render deployment (specific to avoid warnings)
...
app.get('/api/hod/dashboard-summary', authenticateJWT, async (req, res) => {
  try {
    ...
    const departmentStudents = await User.find({ role: 'student', department: hodDepartment }).select('-password');    console.log(`Found ${departmentStudents.length} students in ${hodDepartment}`);
