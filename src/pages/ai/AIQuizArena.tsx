  ...
  const loadPlayerStats = async () => {
    ...
    try {
      ...
      console.log(`📝 Question IDs in history:`, historyArray.slice(0, 5).join(', ') + (historyArray.length > 5 ? '...' : ''));
        ...
        // Check if exact question ID is in history
        if (historyArray.includes(q.id)) {
        ...
        const isContentSimilar = historyArray.some(historyId => {
          ...
          // Check if question text is very similar (over 85% match)
          const similarity = calculateStringSimilarity(q.question.toLowerCase(), historyQuestion.question.toLowerCase());
          if (similarity > 0.85) {
            console.log(`❌ Question ${q.id} too similar to ${historyId} (${Math.round(similarity * 100)}% match)`);
            return true;
          }
          return false;
        });
        ...
        console.log(`✅ Question ${q.id} is fresh and unique`);
        return true;
      });
                        ...
                        <div className="flex items-start space-x-3">
                          <div className="flex-shrink-0 w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center">
                            ...
                            <h4 className="font-semibold mb-2">Explanation</h4>                            <p className="text-blue-100">
                              {(() => {
                                // Enhanced explanation logic with multiple fallbacks
                                if (question.explanation && 
                                    question.explanation.trim() !== '' && 
                                    !question.explanation.includes('tests your knowledge of') &&
                                    question.explanation.length > 20) {
                                  return question.explanation;
                                }
                                
                                // Generate detailed explanation based on category and correct answer
                                const correctAnswer = question.options[question.correctAnswer];
                                const category = question.category || currentSession?.category || 'general';
                                const categoryName = getCategoryDisplayName(category);
                                ...
                                if (category.toLowerCase().includes('javascript')) {
                                  explanation += `In JavaScript, this follows ES6+ standards and modern coding practices. `;
                                } else if (category.toLowerCase().includes('react')) {
                                  explanation += `In React development, this approach ensures proper component lifecycle and state management. `;
                                } else if (category.toLowerCase().includes('algorithm')) {
                                  explanation += `This algorithmic solution provides optimal time and space complexity for the given problem. `;
                                } else if (category.toLowerCase().includes('database')) {
                                  explanation += `This database approach ensures data integrity and follows normalization principles. `;
                                } else if (category.toLowerCase().includes('python')) {
                                  explanation += `This Python solution leverages the language's built-in capabilities efficiently. `;
                                } else if (category.toLowerCase().includes('security')) {
                                  explanation += `This security measure helps protect against common vulnerabilities and attacks. `;
                                } else if (category.toLowerCase().includes('ai')) {
                                  explanation += `This AI concept is fundamental to machine learning and neural network architectures. `;
                                } else {
                                  explanation += `This solution demonstrates best practices in ${categoryName || 'programming'}. `;
                                }
                                
                                // Add additional context based on difficulty
                                if (question.difficulty === 'Hard' || question.difficulty === 'Expert') {
                                  explanation += `This is an advanced concept that requires deep understanding of the underlying principles.`;
                                } else if (question.difficulty === 'Easy') {
                                  explanation += `This fundamental concept is essential for building a strong foundation.`;
                                } else {
                                  explanation += `Understanding this concept will help you tackle more complex problems.`;
                                }
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-6">
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <Card className="bg-white/10 backdrop-blur border-white/20 text-white">
              ...
              <CardContent>
                ...
                <div className="mt-4">
                  ...
                  </div>                  <Progress value={(playerStats.xp % 100)} className="h-2" />
                </div>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div
            ...
            className="mb-6"
          >
            <Card className="bg-orange-500/20 backdrop-blur border-orange-400/30 text-white">
              <CardHeader>
